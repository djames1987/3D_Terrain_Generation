...
from GameV1.lib.ChunkObj import *
from panda3d.core import Vec2, NodePath


class TerrainManager:
    def __init__(self, base, player_pos):
    ...
    def load_chunk(self, x, y):
        pos = Vec2(x, y)
        chunk = ChunkObj(self.base, pos)
        chunk.chunk.setPos(x * (CHUNKSIZE - 1), y * (CHUNKSIZE - 1), 0)
    ...
    def update_chunks(self):
        ...
        for pos in new_live_chunks:
            if pos not in current_live_chunks:
                ...
                self.chunksInRange.append(ChunkObj(self.base, Vec2(pos[0], pos[1])))

        # Buffer any new chunks in the buffer range
        for pos in new_buffered_chunks:
            if pos not in current_buffered_chunks:
                self.buffer_chunk(ChunkObj(self.base, Vec2(pos[0], pos[1])))
                self.chunksInBufferRange.append(ChunkObj(self.base, Vec2(pos[0], pos[1])))
